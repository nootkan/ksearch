...
our ($INDEXER_START, $BASE_URL, $SEARCH_URL, $KSEARCH_DIR, $INDEXER_URL, $PASSWORD);
our ($DESCRIPTION_LENGTH, $DESCRIPTION_START, $USE_DBM, $SAVE_CONTENT, $IGNORE_COMMON_TERMS);
our ($MAKE_LOG, $LOG_SEARCH, $PDF_TO_TEXT, $PDF_INFO, $RESULTS_PER_PAGE, $MIN_TERM_LENGTH);
our ($SHOW_MATCHES, $SHOW_MATCHES_LENGTH, $DO_PHRASES, $CASE_SENSITIVE, $SEARCH_RESULTS);
our ($ALL, $META_DESCRIPTION, $META_KEYWORD, $META_AUTHOR, $ALT_TEXT, $LINKS, $URL);
our ($SORT_BY, $USER_WEIGHTS, $TITLE_WEIGHT, $META_DESCRIPTION_WEIGHT, $META_KEYWORD_WEIGHT);
our ($BOLD_TERMS_BG_COLOR, $BOLD_RESULTS_BG_COLOR, $SEARCH_TITLE, $RESULTS_TITLE);
our ($FORM_INPUT_NAME, $NEXT, $PREVIOUS, $TRANSLATE_CHARACTERS);
our ($DATABASE_DIR, $DATABASEFILE, $F_FILE_DB_FILE, $F_SIZE_DB_FILE, $F_DATE_DB_FILE);
our ($F_TERMCOUNT_DB_FILE, $DESCRIPTIONS_DB_FILE, $TITLES_DB_FILE, $FILENAMES_DB_FILE);
our ($TERMS_DB_FILE, $CONTENTS_DB_FILE, $ALT_TEXT_DB_FILE, $LINKS_DB_FILE);
our ($META_DESCRIPTION_DB_FILE, $META_KEYWORD_DB_FILE, $META_AUTHOR_DB_FILE);
our ($CONFIGURATION_DIR, $IGNORE_FILES_FILE, $IGNORE_TERMS_FILE, $HELP_FILE);
our ($LOG_FILE, $TEMPLATE_DIR, $KSEARCH_TEMPLATE, $FORM_LINK, $SPEED_TIP_TIME, $VERSION);
...
if ($USE_DBM) {
	tie %f_file_db, $dbm_package, $F_FILE_DB_FILE, O_RDONLY, 0644 or die "Cannot open $F_FILE_DB_FILE: $!";
	tie %f_date_db, $dbm_package, $F_DATE_DB_FILE, O_RDONLY, 0644 or die "Cannot open $F_DATE_DB_FILE: $!";
	tie %f_size_db, $dbm_package, $F_SIZE_DB_FILE, O_RDONLY, 0644 or die "Cannot open $F_SIZE_DB_FILE: $!";
	tie %f_termcount_db, $dbm_package, $F_TERMCOUNT_DB_FILE, O_RDONLY, 0644 or die "Cannot open $F_TERMCOUNT_DB_FILE: $!";
	tie %descriptions_db, $dbm_package, $DESCRIPTIONS_DB_FILE, O_RDONLY, 0644 or die "Cannot open $DESCRIPTIONS_DB_FILE: $!";
	tie %titles_db, $dbm_package, $TITLES_DB_FILE, O_RDONLY, 0644 or die "Cannot open $TITLES_DB_FILE: $!";
	tie %filenames_db, $dbm_package, $FILENAMES_DB_FILE, O_RDONLY, 0644 or die "Cannot open $FILENAMES_DB_FILE: $!";
	if ($usehash) {	# get contents from DBM if no key/value size limits
	        tie %contents_db, $dbm_package, $CONTENTS_DB_FILE, O_RDONLY, 0644 or die "Cannot open $CONTENTS_DB_FILE: $!";
	}
	if ($ALT_TEXT) {
		tie %alt_text_db, $dbm_package, $ALT_TEXT_DB_FILE, O_RDONLY, 0644 or die "Cannot open $ALT_TEXT_DB_FILE: $!";
	}
	if ($META_DESCRIPTION) {
		tie %meta_description_db, $dbm_package, $META_DESCRIPTION_DB_FILE, O_RDONLY, 0644 or die "Cannot open $META_DESCRIPTION_DB_FILE: $!";
	}
	if ($META_KEYWORD) {
		tie %meta_keyword_db, $dbm_package, $META_KEYWORD_DB_FILE, O_RDONLY, 0644 or die "Cannot open $META_KEYWORD_DB_FILE: $!";
	}
	if ($META_AUTHOR) {
		tie %meta_author_db, $dbm_package, $META_AUTHOR_DB_FILE, O_RDONLY, 0644 or die "Cannot open $META_AUTHOR_DB_FILE: $!";
	}
	if ($LINKS) {
	        tie %links_db, $dbm_package, $LINKS_DB_FILE, O_RDONLY, 0644 or die "Cannot open $LINKS_DB_FILE: $!";
	}
}
...
my $show_matches;
# set sorting choice, results per page with validation
my $sort_param = $query->param('sort') || '';
if ($sort_param eq "Scores" || $sort_param eq "Dates" || $sort_param eq "Matches" || 
    $sort_param eq "Sizes" || $sort_param eq "Titles" || $sort_param eq "File Names") {
    $SORT_BY = $sort_param;
}

my $display_param = $query->param('display') || 0;
if ($display_param >= 5 && $display_param <= 100) {
    $RESULTS_PER_PAGE = int($display_param);
}

$show_matches = $query->param('showm') || 0;
$show_matches = int($show_matches) if $show_matches =~ /^\d+$/;

# to search within previous results - with validation
if ($SEARCH_RESULTS && $query->param('pq') && $query->param($FORM_INPUT_NAME) && 
    $query->param('help') != 1) {
	my $pq = $query->param('pq');
	if ($pq) {
		$previous_queries = $pq.'  ';
		@previous_queries = split "  ", CGI::unescape($pq);
	}
}
...
sub start_search {
	...
	if (@previous_queries) { # search results of previous queries
		...
		$query_terms = 'all:'.$query_terms if $query->param('all') && $query->param('all') == 1;
		$query_terms = 'c:'.$query_terms if ($query->param('c') && $query->param('c') eq "s" && $CASE_SENSITIVE);
		$query_terms = 'w:'.$query_terms if $query->param('w') && $query->param('w') == 1;
		$query_terms = 'st:'.$query_terms if ($query->param('st') && $query->param('st') == 1 && $ALL);
		unless ($query->param('default') && $query->param('default') == 1) { # search content options
			$query_terms = 'b:'.$query_terms if $query->param('b') && $query->param('b') == 1;
			$query_terms = 't:'.$query_terms if ($query->param('t') && $query->param('t') == 1);
			$query_terms = 'd:'.$query_terms if ($query->param('d') && $query->param('d') == 1 && $META_DESCRIPTION);
			$query_terms = 'k:'.$query_terms if ($query->param('k') && $query->param('k') == 1 && $META_KEYWORD);
			$query_terms = 'au:'.$query_terms if ($query->param('au') && $query->param('au') == 1 && $META_AUTHOR);
			$query_terms = 'alt:'.$query_terms if ($query->param('alt') && $query->param('alt') == 1 && $ALT_TEXT);
			$query_terms = 'l:'.$query_terms if ($query->param('l') && $query->param('l') == 1 && $LINKS);
			$query_terms = 'u:'.$query_terms if ($query->param('u') && $query->param('u') == 1 && $URL);
		}
		$query_terms =~ s/^\s+//;
		$query_terms =~ s/\s+$//;
		$query_terms =~ s/\s+/ /g;
	...
	if ($query->param('exact') && $query->param('exact') == 1 && $query_terms !~ /^".*"$/) {
		$query_terms = '"'.$query_terms.'"';
	}
	returnresults() if ($query->param('help') && $query->param('help') == 1 || $query_terms =~ /^\s*$/);
...
sub search_files {
	if ($USE_DBM) {
		while (my ($file, $file_path) = each(%f_file_db)) {
		...
		foreach (<FILEDB>) {
			...
			($f_file_db{$file_count}, $filenames_db{$file_count}, $f_date_db{$file_count},
			 $f_size_db{$file_count}, $f_termcount_db{$file_count}, $descriptions_db{$file_count},
			 $titles_db{$file_count}, $contents_db{$file_count}, $alt_text_db{$file_count},
			 $meta_description_db{$file_count}, $meta_keyword_db{$file_count}, 
			 $meta_author_db{$file_count}, $links_db{$file_count}) = @fields;
			
			search_contents($file_count, $f_file_db{$file_count});
		}
		close(FILEDB);
	}
}
...
sub get_sorted_answers {
	...
	foreach my $answer (@final_files) {		# get final answers
			...
			$totalmatches += $matches{$answer} || 0;
			$totalsize += $f_size_db{$answer} || 0;
		}
	}
	...
	if ($SORT_BY eq "Matches") {  # sort answers
        	  @sortedanswers = sort {($matches{$b} || 0) <=> ($matches{$a} || 0)
                                           ||
                        ($finalscores{$b} || 0) <=> ($finalscores{$a} || 0)
                                           ||
                        ($f_date_db{$b} || 0) <=> ($f_date_db{$a} || 0)
                                           ||
                        ($f_size_db{$b} || 0) <=> ($f_size_db{$a} || 0)
                        		   ||
 			lc($titles_db{$a} || '') cmp lc($titles_db{$b} || '')
                        		   ||
 			lc($filenames_db{$a} || '') cmp lc($filenames_db{$b} || '') } keys %finalscores;
  	} elsif ($SORT_BY eq "Scores") {
        	  @sortedanswers = sort {($finalscores{$b} || 0) <=> ($finalscores{$a} || 0)
                                           ||
                        ($matches{$b} || 0) <=> ($matches{$a} || 0)
                                           ||
                        ($f_date_db{$b} || 0) <=> ($f_date_db{$a} || 0)
                                           ||
                        ($f_size_db{$b} || 0) <=> ($f_size_db{$a} || 0)
 			...
 			lc($filenames_db{$a} || '') cmp lc($filenames_db{$b} || '') } keys %finalscores;
  	        ...
  	        @sortedanswers = sort {($f_date_db{$b} || 0) <=> ($f_date_db{$a} || 0)
                                           ||
                        ($matches{$b} || 0) <=> ($matches{$a} || 0)
                                           ||
                        ($finalscores{$b} || 0) <=> ($finalscores{$a} || 0)
 			...
 			lc($filenames_db{$a} || '') cmp lc($filenames_db{$b} || '') } keys %finalscores;
        	  ...
        	  @sortedanswers = sort {($f_size_db{$b} || 0) <=> ($f_size_db{$a} || 0)
                                           ||
                        ($matches{$b} || 0) <=> ($matches{$a} || 0)
                                           ||
                        ($finalscores{$b} || 0) <=> ($finalscores{$a} || 0)
 			...
 			lc($filenames_db{$a} || '') cmp lc($filenames_db{$b} || '')
                        ...
                        ($f_date_db{$b} || 0) <=> ($f_date_db{$a} || 0)
                        		   ||
 			($f_size_db{$b} || 0) <=> ($f_size_db{$a} || 0) } keys %finalscores;
  	} else {
...
sub returnresults {	# creates HTML page from template file
	...
	my $currentpage = $query->param('p') || 1;
	$currentpage = int($currentpage) if $currentpage =~ /^\d+$/;
	$currentpage = 1 if $currentpage < 1;
	
	if ($SEARCH_RESULTS && $query->param('pq') && $query->param('pq') !~ /^\s*$/) {
		my $safe_pq = $query->param('pq');
		if ($safe_pq) {
			$subsearch_string = '&pq='.CGI::escape($safe_pq);
	...
	if ($query->param('exact') && $query->param('exact') == 1) {
		$h{exact} = '<input type="checkbox" name="exact" value="1" checked="checked" />Exact Match';
	} else {
		$h{exact} = '<input type="checkbox" name="exact" value="1" />Exact Match';
	}
	if ($all) {
		$options .= '&amp;st=1';
	...
	$search_sources = '' unless defined $search_sources;
	$search_sources =~ s/,$//;
	...
	if ($RESULTS_PER_PAGE == 5) {
		...
		$h{display} = '<option selected="selected">'.$RESULTS_PER_PAGE.'</option><option>5</option> <option>10</option> <option>25</option> <option>50</option> <option>100</option>';
	}
	...
	my $ww = $whole_word ? ' whole words in' : '';
	$search_sources = "<br />Searched$ww: <strong>$search_sources</strong>";
	if ($query->param('help') && $query->param('help') == 1) {
		...
		$h{header} = 'Your '.$casesearch.' search for <strong>'.$bare_query.'</strong> did not match any documents'.($subsearch_info || '');
		$h{subheader} = '<br />No pages were found containing: <strong>'.(join " ", @none).'</strong>'.$commonterms.$search_sources.($weight_tip || '');
		$h{help} = $SEARCH_TITLE || '' if $SEARCH_TITLE;
	} elsif (!$results) {
		$h{header} = 'Your '.$casesearch.' search for <strong>'.$bare_query.'</strong> did not match any documents'.($subsearch_info || '');
		...
		my $documents;
		if ($first == $last) {
			$documents = 'Result <strong>'.($first+1).'</strong>';
		} else {
			$documents = 'Results <strong>'.($first+1).'-'.($last+1).'</strong>';
		}
		$h{help} = $RESULTS_TITLE || '' if $RESULTS_TITLE;
		my $s = ($totalmatches == 1 ? "" : "es");
		$h{header} = $documents.' of <strong>'.$results.' ('.$totalsize.'KB)</strong> for the word "<strong>'.$bare_query.'</strong>" with <strong>'.$totalmatches.'</strong> total '.$casesearch.' match'.$s.($subsearch_info || '');
		$h{subheader} = $commonterms.$search_sources.($weight_tip || '');
		$h{results} = $documents.' of <strong>'.$results.'</strong>';
		my $checked = ' checked' if $query->param('pq') && $query->param('pq') !~ /^\s*$/;
		my $safe_pq_val = $previous_queries.$previous_query_str;
		$h{subsearch} = '<input type="checkbox" name="pq" value="'.$safe_pq_val.'"'.$checked.' />Search Within Results' if $SEARCH_RESULTS;
		$h{form_link} = $FORM_LINK || '';
	}
	#################### Header Format End ####################
	# Display Search Results (loop)
	my @results_array;
	foreach my $result ((@sortedanswers)[$first..$last]) {
		next unless defined $result;
		my ($title, $desc);
		if ($show_matches) {
			$desc = show_matches(\@checked_terms, $result);
		} else {
			$desc = (($meta_description_db{$result} && $search_meta_description) ? 
					 $meta_description_db{$result} : $descriptions_db{$result}) || '';
		}
		if ($SORT_BY eq "File Names") {
		   $title = $filenames_db{$result} || '';
		} else {
		   $title = $titles_db{$result} || '';
		}
		my ($scoreA, $scoreB, $scoreC, $scoreD);
		if ($SORT_BY eq "Matches") {
			$scoreA = '<strong>Matches:</strong> '.($matches{$result} || 0);
			$scoreB = "<strong>$score</strong> ".($finalscores{$result} || 0);
			$scoreC = '<strong>Last Updated:</strong> '.get_date($f_date_db{$result} || 0);
			$scoreD = '<strong>File Size:</strong> '.($f_size_db{$result} || 0).'KB';
		} elsif ($SORT_BY eq "Scores") {
			$scoreA = "<strong>$score</strong> ".($finalscores{$result} || 0);
			$scoreB = '<strong>Matches:</strong> '.($matches{$result} || 0);
			$scoreC = '<strong>Last Updated:</strong> '.get_date($f_date_db{$result} || 0);
			$scoreD = '<strong>File Size:</strong> '.($f_size_db{$result} || 0).'KB';
		} elsif ($SORT_BY eq "Dates")  {
			$scoreA = '<strong>Last Updated:</strong> '.get_date($f_date_db{$result} || 0);
			$scoreB = '<strong>Matches:</strong> '.($matches{$result} || 0);
			$scoreC = "<strong>$score</strong> ".($finalscores{$result} || 0);
			$scoreD = '<strong>File Size:</strong> '.($f_size_db{$result} || 0).'KB';
		} else {
			$scoreA = '<strong>Matches:</strong> '.($matches{$result} || 0);
			$scoreB = '<strong>File Size:</strong> '.($f_size_db{$result} || 0).'KB';
			$scoreC = "<strong>$score</strong> ".($finalscores{$result} || 0);
			$scoreD = '<strong>Last Updated:</strong> '.get_date($f_date_db{$result} || 0);
		}
		my $display_url = $BASE_URL.($f_file_db{$result} || '');
		...
		if ($PDF_TO_TEXT && $PDF_INFO && ($f_file_db{$result} || '') =~ m/\.pdf$/i) {
			$title = $title.' &nbsp; - &nbsp; <strong><em>PDF '.($f_size_db{$result} || 0).'KB</em></strong>';
		}

		$desc = $desc.'<br />' if $desc !~ /^\s*$/;
		push @results_array, {rank => $first+(++$rank),
                                        url => $BASE_URL.($f_file_db{$result} || ''),
	...
	if ($query->param('help') && $query->param('help') == 1) {	# add 'back' link to help page
...
#	if (timestr(timediff(new Benchmark, $t0)) =~ /^(.+)$/i && !@none && $results) { # wallclock, usr, and sys times
	...
	if ( $bare_query !~ /^\s*$/ && $LOG_SEARCH ) {
		...
		print FILE "REMOTE IP: ".($ENV{'REMOTE_ADDR'} || 'unknown')."\n";
		print FILE "REMOTE HOST: ".($ENV{'REMOTE_HOST'} || 'unknown')."\n";
...
sub find_matches {	# slow part
	...
	return 0 unless defined $content && defined $term;
	my $matches = 0;
...
sub ignore_terms {
  ...
  open (FILE, '<', $IGNORE_TERMS_FILE) or (warn "Cannot open $IGNORE_TERMS_FILE: $!" and return '');
  while (<FILE>) {
    chomp;
    $_ =~ s/#.*$//g;
    $_ =~ s/\s//g;
...
sub get_date {  # gets date of last modification
   my $updatetime = $_[0] || 0;
   my $real_time = $updatetime * 8640;
   my @month = ('Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec');
   my ($mday,$mon,$yr) = (localtime($real_time))[3,4,5];
   $yr += 1900;
...
sub template_results {
   ...
   foreach (@{$results_values}) {
     $loop = $loop_copy;
     $loop =~ s/<!--\s*item:\s*(\S+?)\s*-->/$_->{$1} || ''/gise;
     $out .= $loop;
   }
   $html =~ s/(<!--\s*loop:\s*results\s*-->).*?(<!--\s*end:\s*results\s*-->)/$1$out$2/s;
}
sub show_matches {
		...
		if ($whole_word) {
			if ($case_sensitive) {
				while ($count < $show_matches && $bdy =~ /\b$boldterm\b/gs) {
					...
					$post =~ m/^(.{0,$LENGTH})\b/; $postm = $1; $post = $postm;
					$bdy = "$pre $post";
