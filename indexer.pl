...
sub validate_file_path {
    ...
    return '' if $path =~ /^[\/\\]/;  # No absolute paths
...
sub indexer {
  ...
  return unless $safe_dir;
  
  chdir $safe_dir or (warn "Cannot chdir $safe_dir: $!\n" and return);
  opendir(DIR, $safe_dir) or (warn "Cannot open $safe_dir: $!\n" and return);
...
sub index_file {
  ...
  if($PDF_TO_TEXT && $file =~ m/\.pdf$/i) {	# if the file is a PDF file
    ...
    undef $/;
    open(FILE, '<', $file) or (warn "Cannot open $file: $!" and return);
    $contents = <FILE>;
    close(FILE);
    $/ = "\n";
  }

  if (!$contents || $contents =~ /^\s*$/gs) { return; } # skip empty files
